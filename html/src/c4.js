...
function callTab(){
	...
	function resetGame(){
		if (currentLobby[0] == "B"){
		ws.send(JSON.stringify({func:".c4.runJobBot",args:{arg:"Y",lobby:currentLobby[1]}}));
		} else {
	...
	function createGrid(x){
		var grid = clickableGrid(x,function(el,row,col,i){
			...
			if (currentLobby[0] == "B"){
			ws.send(JSON.stringify({func:".c4.runJobBot",args:{arg:x[row][0].split(">")[1][0]+col,lobby:currentLobby[1]}}));
			} else {
	...
	function changeChannel(channel){
		if (ws == null){
			...
			if (currentLobby != null){
				if (currentLobby[0] == "B"){
					var msg=JSON.stringify({func:".c4.leaveLobbyBot",args:currentLobby[1]});
				} else {var msg=JSON.stringify({func:".c4.leaveLobby",args:currentLobby});}
				$("#channelID").text("You have left channel "+currentLobby);
				ws.send(msg);
			}
			currentLobby=channel;
			if (channel[0] == "B") {
				var msg=JSON.stringify({func:".c4.joinLobbyBot",args:channel[1]});
			} else {var msg=JSON.stringify({func:".c4.joinLobby",args:channel});}
